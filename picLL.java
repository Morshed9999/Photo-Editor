...
class ImgArea extends Canvas {
    ...
    public ImgArea() {
        addMouseListener(new ImgArea.Mousexy()); //hanlding mouse event of Canvas class
        addKeyListener(new ImgArea.KList()); //handling key event of the Canvas
        try {
...
//draw the update image
    ...
    private void makeCompression(File file) {
...
//so the image is brighter or darker
    ...
    public void filterImage() {
        ...
        ConvolveOp cop = new ConvolveOp(kernel, ConvolveOp.EDGE_NO_OP, null); //create ConvolveOp to
//encapsulate
//the kernel
...
class Main extends JFrame implements ActionListener {
    ...
    Main() {
        ...
        mresize.setMnemonic(KeyEvent.VK_R); //chng
        mresize.addActionListener(this);

        mcompress = new JMenuItem("Zoom");
        mcompress.setMnemonic(KeyEvent.VK_P);
    //setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        mcompress.addActionListener(new ActionListener() {

            @Override
            public void actionPerformed(ActionEvent e) {
              //  MainInter o=new MainInter();
                setVisible(false);
              //  o.setVisible(true);
...
//setDefaultCloseOperation(EXIT_ON_CLOSE);
            ...
            txtWidth.addKeyListener(new Main.ImageResize.KeyList());
            txtHeight = new JTextField(4);
            txtHeight.addKeyListener(new Main.ImageResize.KeyList());
            panel = new JPanel();
...
//The TextAdd class is in the Main class
    ...
    public class TextAdd extends JFrame implements ActionListener {
        ...
        TextAdd() {
            colorText = null;
            setTitle("Add text to the imaage");
//setDefaultCloseOperation(EXIT_ON_CLOSE);
...
//handling events of sub-menu items on the main program interface
    ...
    public void mcompressactionPerformed(ActionEvent e) {
        JMenuItem source = (JMenuItem) (e.getSource());
          if (source.getText().compareTo("Zoom") == 0) {
           // new MainInter();
        }
        }
        ...
        JMenuItem source = (JMenuItem) (e.getSource());
         if (source.getText().compareTo("Open...") == 0) {
          ...
          ia.repaint();
            ...
            Main.ImageBrightness ib = new Main.ImageBrightness();
            if (ImgArea.imageLoaded) {
                ib.enableSlider(true);
            }
        } else if (source.getText().compareTo("Zoom") == 0) {
            if (ImgArea.imageLoaded) {
                ia.setActionCompressed(true);
                enableSaving(true);
            }
        } else if (source.getText().compareTo("Image resize") == 0) {
            Main.ImageResize ir = new Main.ImageResize();
            if (ImgArea.imageLoaded) {
...
public class picLL {
    ...
    public static void main(String args[]) {
        ...
        new Main();
//    MainInterrr ob= new MainInterrr();
        //   ob.setVisible(true);
//   new Zoom();
        //new MainInterrr();
//Zoom vv=new Zoom();
